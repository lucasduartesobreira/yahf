...
pub struct HyperServer<PreM, AfterM> {
    router: Router<PreM, AfterM>,
}

impl<PreM, FutP, ResultP, AfterM, FutA, ResultA> Deref for HyperServer<PreM, AfterM>
where
...
impl<PreM, FutP, ResultP, AfterM, FutA, ResultA> DerefMut for HyperServer<PreM, AfterM>
where
    PreM: PreMiddleware<FutCallResponse = FutP>,
    FutP: Future<Output = ResultP>,
    ResultP: Into<InternalResult<Request<String>>>,
...
impl<PreM, FutP, ResultP, AfterM, FutA, ResultA> HyperServer<PreM, AfterM>
where
    ...
    ResultA: Into<InternalResult<Response<String>>> + std::marker::Send + 'static,
{
    ...
    pub fn pre<NewPreM, NewFut, NewResultP>(
        self,
        middleware: NewPreM,
    ) -> HyperServer<impl PreMiddleware<FutCallResponse = impl Future<Output = NewResultP>>, AfterM>
    where
        ...
        NewResultP: Into<InternalResult<Request<String>>>,
    {
        ...
        HyperServer { router: new_router }
    }

    pub fn after<NewAfterM, NewFut, NewResultA>(
        self,
        middleware: NewAfterM,
    ) -> HyperServer<PreM, impl AfterMiddleware<FutCallResponse = impl Future<Output = NewResultA>>>
    where
...
async fn handle_req<PreM, FutP, ResultP, AfterM, FutA, ResultA>(
    server: Arc<HyperServer<PreM, AfterM>>,
